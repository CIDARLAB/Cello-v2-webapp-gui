      ...
      o = function () {
        ...
        function n(e, t) {
          return {
            ...
            next: [
              ...
              { regex: e + (t.multiline ? '' : '|$'), token: 'string.end', next: t.next || 'start' },
              { defaultToken: 'string' },
            ],
          };
        }
        ...
        (this.$rules = {
          start: [
            ...
            { regex: /@[a-zA-Z_$][a-zA-Z_$\d\u0080-\ufffe]*/, token: 'constant.language' },
            { regex: /[a-zA-Z_$][a-zA-Z_$\d\u0080-\ufffe]*/, token: t },
            { regex: '`#%', token: 'error.invalid' },
            { token: 'constant.numeric', regex: /[+-]?\d+(?:(?:\.\d*)?(?:[LlDdEe][+-]?\d+)?)\b|\.\d+[LlDdEe]/ },
            {
            ...
            { token: 'paren.lparen', regex: /[\[]/, next: 'maybe_soql', merge: !1 },
            { token: 'paren.lparen', regex: /[\[({]/, next: 'start', merge: !1 },
            { token: 'paren.rparen', regex: /[\])}]/, merge: !1 },
          ],
          maybe_soql: [
            { regex: /\s+/, token: 'text' },
            { regex: /(SELECT|FIND)\b/, token: 'keyword', caseInsensitive: !0, next: 'soql' },
          ...
          'log-start': [
            { token: 'timestamp.invisible', regex: /^[\d:.() ]+\|/, next: 'log-header' },
            { token: 'timestamp.invisible', regex: /^  (Number of|Maximum)[^:]*:/, next: 'log-comment' },
            { token: 'invisible', regex: /^Execute Anonymous:/, next: 'log-comment' },
            { defaultToken: 'text' },
          ],
          'log-comment': [{ token: 'log-comment', regex: /.*$/, next: 'log-start' }],
          'log-header': [
            { token: 'timestamp.invisible', regex: /((USER_DEBUG|\[\d+\]|DEBUG)\|)+/ },
            {
              token: 'keyword',
              regex:
