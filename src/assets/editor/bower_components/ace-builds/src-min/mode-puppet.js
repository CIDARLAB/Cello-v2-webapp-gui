    ...
    s = function () {
      (this.$rules = {
        start: [
          ...
          { token: ['language.support.class', 'keyword.operator'], regex: '\\b([a-zA-Z_]+)(\\s+=>)' },
          {
          ...
          { token: 'multiline.comment.begin.puppet', regex: '^\\s*\\/\\*\\s*$', push: 'blockComment' },
          { token: 'keyword.control.puppet', regex: '\\b(case|if|unless|else|elsif|in|default:|and|or)\\s+(?!::)' },
          {
        ...
        blockComment: [
          { regex: '^\\s*\\/\\*\\s*$', token: 'multiline.comment.begin.puppet', push: 'blockComment' },
          { regex: '^\\s*\\*\\/\\s*$', token: 'multiline.comment.end.puppet', next: 'pop' },
