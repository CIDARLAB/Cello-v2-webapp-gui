...
(function (load) {
  ...
  var requirePath =
    self.Smoothie && self.Smoothie.requirePath !== undefined ? self.Smoothie.requirePath.slice(0) : ['./'];
  var requireCompiler =
    self.Smoothie && self.Smoothie.requireCompiler !== undefined ? self.Smoothie.requireCompiler : null;
  ...
  for (var i = 0; i < requirePath.length; i++) {
    ...
    else parser.href = requirePath[i];
  ...
  function require(identifier, callback, compiler) {
    ...
    if (callback) request.onload = onLoad;
    ...
    function onLoad() {
      if (lock[cacheid]) {
        ...
        if (request.status != 200)
          throw new SmoothieError(
            'unable to load ' + descriptor.id + ' (' + request.status + ' ' + request.statusText + ')'
          );
        if (!cache[cacheid]) {
          var source = compiler ? compiler(request.responseText) : request.responseText;
          load(descriptor, cache, pwd, 'function(){\n' + source + '\n}');
        }
        if (callback) callback(cache[cacheid]);
      }
    }
  }
  ...
  function resolve(identifier) {
    ...
    if (uri.substr(0, root.length) != root) throw new SmoothieError('relative identifier outside of module root');
  ...
  if (self.require !== undefined) throw new SmoothieError("'require' already defined in global scope");
  ...
  function (/*load*/ module /*, cache, pwd, source*/) {
    ...
    if (typeof module.id !== 'string')
      for (var id in module) arguments[1]['$' + require.resolve(id).id] = module[id].toString();
    arguments[2].shift();
  }
);
