    ...
    s = function () {
      (this.$rules = {
        start: [
          ...
          { token: 'constant.numeric.fslProbability', regex: /[0-9]+%/, comment: 'edge probability annotation' },
          { token: 'constant.character.fslAction', regex: /\'[^']*\'/, comment: 'action annotation' },
          { token: 'string.quoted.double.fslLabel.doublequoted', regex: /\"[^"]*\"/, comment: 'fsl label annotation' },
          { token: 'entity.name.tag.fslLabel.atom', regex: /[a-zA-Z0-9_.+&()#@!?,]/, comment: 'fsl label annotation' },
        ],
      }),
        this.normalizeRules();
    };
